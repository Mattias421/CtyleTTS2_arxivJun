...
from Modules.cde import NeuralCDE
...
def build_model(args, text_aligner, pitch_extractor, bert):
    ...
    cde_cfg = getattr(args, "cde", None)
    if cde_cfg is not None and bool(getattr(cde_cfg, "enabled", False)):
        nets.cde = NeuralCDE(
            channels=args.hidden_dim,
            hidden_channels=getattr(cde_cfg, "hidden_channels", 256),
            interpolation=getattr(cde_cfg, "interpolation", "linear"),
            solver=getattr(cde_cfg, "solver", "reversible_heun"),
            dt=getattr(cde_cfg, "dt", 1.0),
            atol=getattr(cde_cfg, "atol", 1e-5),
            rtol=getattr(cde_cfg, "rtol", 1e-5),
        )

    return nets


def maybe_apply_cde(model, t_en, text_mask, durations=None):
